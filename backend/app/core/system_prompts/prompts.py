...
Users are recommended to modify their servers as is done in this template
Note that we import Streamable HTTP from Anthropic’s MCP.
...
Package.json points to compiled version
Build script makes output executable
No root-level index.ts needed
​
.gitignore
...
Here is also a real working example that shows how toa ctually build a MCP server: 
{example_mcp}


and here its your soruce of turht of how to build MCP servers:
