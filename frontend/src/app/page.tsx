...
const demoSSEEvents: SSEEvent[] = [
  { type: 'log', content: '🔍 Analyzing requirements...', timestamp: Date.now() },
  { type: 'log', content: '📋 Creating project structure...', timestamp: Date.now() + 1000 },
  { type: 'code', content: 'manifest.json', timestamp: Date.now() + 2000 },
  { type: 'log', content: '⚡ Generating server code...', timestamp: Date.now() + 3000 },
  { type: 'code', content: 'server.py', timestamp: Date.now() + 4000 },
  { type: 'log', content: '🧪 Running tests...', timestamp: Date.now() + 5000 },
  { type: 'log', content: '✅ All tests passed!', timestamp: Date.now() + 6000 },
]

const buildSteps: BuildStep[] = [
  { id: 'scope', title: 'Scope & Clarify', status: 'queued', description: 'Understanding requirements' },
  { id: 'design', title: 'Design & Scaffolding', status: 'queued', description: 'Planning architecture' },
  { id: 'code', title: 'Code Generation', status: 'queued', description: 'Writing implementation' },
  { id: 'build', title: 'Build & Test', status: 'queued', description: 'Testing and validation' },
  { id: 'ship', title: 'Ship', status: 'queued', description: 'Ready for deployment' },
]
...
function PromptCard({ onPromptSubmit }: { onPromptSubmit: (prompt: string) => void }) {
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 via-white/[0.02] to-white/5 backdrop-blur-xl p-8">
        ...
        <div className="relative z-10">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-white mb-2">Describe what you want to build</h2>
            <p className="text-sm text-white/60 max-w-2xl mx-auto">
              Tip: Tell us the source APIs, the commands you want, and any config you need.
            </p>
          </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-white">More Example Prompts</h3>
                <button
                  onClick={() => setShowMoreExamples(false)}
            ...
            {buildSteps.map((step, index) => (
              ...
              >
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  step.status === 'queued' ? 'bg-white/10 text-white/60' :
                  step.status === 'running' ? 'bg-blue-500/20 text-blue-300' :
                  step.status === 'success' ? 'bg-green-500/20 text-green-300' :
                  step.status === 'needs_input' ? 'bg-yellow-500/20 text-yellow-300' :
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-white">Iterate on your MCP</h3>
                <button
                  onClick={() => setShowIterateDialog(false)}
                  className="p-2 rounded-lg hover:bg-white/10 transition-colors"
                >
              ...
              <div className="flex justify-end gap-3">
                <Button variant="ghost" onClick={() => setShowIterateDialog(false)}>
                  Cancel
                </Button>
      ...
      <main className="relative">
        ...
        <section className="pt-20 pb-16 px-4">
          <div className="max-w-6xl mx-auto text-center">
            ...
            >
              Describe your tool. We plan it, ask smart questions, stream code over SSE, and ship a working MCP you can iterate on.
            </motion.p>
